...
//           structure that data dependence violation is tracked for iterative speculations. 
//           Note that the reference of this data is not included, because the data per se 
//           is only referenced at a higher management level (the speculators or the main section). 
//
struct _data_iterative_access_log{
	...
	bool _output_violations_are_false(void* value_to_write, int pos){ //this function resolves if the output dependence
	...
	//there is book-keeping of the values that are being written, but not of the values that are being read.
		...
		int previous_false_positive_on_the_data=-2; 
		int former_pos=-1;

		for (int i=0; i<_writers.size(); i++){ 
			if (_writers[i]<=pos && _writers[i]>=former_pos){ 
				memcpy((void*)prev_value, &_previous_values[i], _size); 
				former_pos=_writers[i];				
                                 //Initialization of prev_value to the value previous to the current write...
			}
		} //Since the previous values might be disordered, it is necessary to iterate through the whole array...

		for (int i=0; i<_writers.size(); i++){ 
			if (_writers[i]>pos){ //Possible output violation.

                        //Now we need to check if the previous value of the higher iteration that wrote is the same as the one being written.
                        //If that is the case, then a chain of higher iterations who all have the same previous value would be valid
                        //(a chain of false positives). However, if any has a different previous value, then it might be necessary
                        //to restart the higher threads in order to set straight the record keeping. Another alternative might be possible, but
                        //we decided this would be the simpler approach.

				if (memcmp(&value_to_write,_previous_values[i],_size)==0){ //If the value to write is the same as the previous value
											//of the higher iteration, then it could be a false positive...

					if (previous_false_positive_on_the_data==-2){  //If there have been no false positives on the data item...

						if (memcmp((void*)_previous_values[i],&prev_value,_size)!=0){ //If the previous value
							free (prev_value);
							return false;
						}
						...
						if (memcmp((void*)_previous_values[i],&prev_value,_size)!=0){
							free (prev_value);
							return false;
						}
					}
		...
		if (first_reader==-2){
			...
			memcpy ((void*)prev_value, &value_to_write, _size);
...
/*TEMPORALMENTE QUITADO PQ NO ESTOY SEGURO DE LO QUE HACE:
	...
	//and the higher readers have to be restarted.
		...
		void* a; //This is only done to avoid a possible warning from the compiler, arising due to uncollected return from realloc.
		if (_readers.empty())
			return to_cancel;
			...
			for (unsigned int i=0; i<to_cancel.size(); i++){
				...
				while((!_writers.empty())&& j<_writers.size()){
					if (to_cancel[i]==_writers[j]){
						...
						a=realloc (_previous_values[j], 0);
...
//			cout<<"Retorna: "<<*(double*)value<<" "<<value<<endl;
	...
	//value has to be restored to it's state before pos, so it can be read.
		...
		while ((!_writers.empty())&& (i<_writers.size())){
			...
			if (_writers[i]>pos){
				...
				a=realloc (_previous_values[i], 0);
	...
	//A lazy destruction is done of the _previous_values[i] created on the heap. When the container gets deleted, they become unreferenced values, 	
	//candidates for garbage recollection.
};
