...
struct _data_access_log{
	...
	//there is book-keeping of the values that are being written, but not of the values that are being read.
		...
		T* prev_value= (T*) malloc (sizeof(T));
		...
		if (_is_active){
			...
			if (current_thread>=-1){//is a valid read.
				...
				//if the data has never been accesed, it's log should be created
					...
					T* previous_value_aux= (T*) malloc (sizeof(T));

					memcpy ((void*)previous_value_aux, (void*)data_to_be_read, sizeof (T));

				//	cout<<"(R)Previous value: "<<*previous_value_aux<<" "<<(void*)previous_value_aux<<" "<<(T)*data_to_be_read<<" size: "<<sizeof(T)<<endl;

					temp2._previous_values.push_back((void*)&previous_value_aux);
		...
		if (_is_active){
			...
			if (current_thread>=-1){
				...
				//if data has no log entry, then it's entry should be created.
					...
					T* previous_value_aux= (T*) malloc (sizeof (T));;
					memcpy ((void*)previous_value_aux, (void*)data_to_be_written_upon, sizeof(T));
			...
			//if the log exists, then possible true dependence and output dependence violations are checked (RAW and WAW)
				...
				if (memcmp((void*)data_to_be_written_upon, (void*)data_to_write, ptr->_size)==0){
				...
				//So, we will evaluate false positives in output dependence violations.
					...
					memcpy((void*)&value, (void*) ptr->_get_previous_value(current_thread), sizeof (T));
				...
				//		cout<<"Entro a invocar aca, con data to write: "<<data_to_write<<" "<<*data_to_write<<" DTBWU: "<<data_to_be_written_upon<<" "<<&data_to_be_written_upon<<" "<<*data_to_be_written_upon<<endl;
					memcpy((void*)&value, (void*) ptr->_get_previous_value(current_thread), sizeof (T));
					writers_to_cancel=ptr->_cancel_higher_writers(current_thread);
				...
				//those writers have to be restarted
					...
					T* former_value= (T*) malloc (sizeof (T));;
					*former_value=value;
					ptr->_previous_values.push_back((void*)former_value);
				}
