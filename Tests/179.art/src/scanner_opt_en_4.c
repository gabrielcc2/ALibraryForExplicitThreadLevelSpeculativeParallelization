...
int set_high[2];
double tnorm, tnorm1, tnorm2, tnorm3, tnorm4;
parallel_loop_in_batches loop1, loop2, loop3, loop4, loop5, loop6, loop7;
double xr,qr;
int tresult;
...
void* prefetch_for_floop1 (void* arg){
		int i=*(int*)arg;
	       __builtin_prefetch (&a, 0, 2);
               __builtin_prefetch (&f1_layer[i].W, 1, 1);
               __builtin_prefetch (&f1_layer[i].I[cp], 0, 1);
               __builtin_prefetch (&f1_layer[i].U, 0, 1);
               __builtin_prefetch (&f1_layer[i+1].W, 1, 1);
               __builtin_prefetch (&f1_layer[i+1].I[cp], 0, 1);
               __builtin_prefetch (&f1_layer[i+1].U, 0, 1);
               __builtin_prefetch (&f1_layer[i+2].W, 1, 1);
               __builtin_prefetch (&f1_layer[i+2].I[cp], 0, 1);
               __builtin_prefetch (&f1_layer[i+2].U, 0, 1);
               __builtin_prefetch (&f1_layer[i+3].W, 1, 1);
               __builtin_prefetch (&f1_layer[i+3].I[cp], 0, 1);
               __builtin_prefetch (&f1_layer[i+3].U, 0, 1);
               __builtin_prefetch (&f1_layer[i+4].W, 1, 1);
               __builtin_prefetch (&f1_layer[i+4].I[cp], 0, 1);
               __builtin_prefetch (&f1_layer[i+4].U, 0, 1);
}
void* floop1 (void* arg){
      int tope= *(int*)arg;
      double tnorm_aux=0;
  //    ht_speculator helper_speculator;
      for (int i=tope-2500; i<tope;i++)
      {
	 /* if (i%5==0){
//		aux_i=i;
//		helper_speculator.speculate(prefetch_for_floop1, (void*)&aux_i);
	       __builtin_prefetch (&a, 0, 2);
               __builtin_prefetch (&f1_layer[i].W, 1, 1);
               __builtin_prefetch (&f1_layer[i].I[cp], 0, 0);
               __builtin_prefetch (&f1_layer[i].U, 0, 0);
               __builtin_prefetch (&f1_layer[i+1].W, 1, 1);
               __builtin_prefetch (&f1_layer[i+1].I[cp], 0, 0);
               __builtin_prefetch (&f1_layer[i+1].U, 0, 0);
               __builtin_prefetch (&f1_layer[i+2].W, 1, 1);
               __builtin_prefetch (&f1_layer[i+2].I[cp], 0, 0);
               __builtin_prefetch (&f1_layer[i+2].U, 0, 0);
               __builtin_prefetch (&f1_layer[i+3].W, 1, 1);
               __builtin_prefetch (&f1_layer[i+3].I[cp], 0, 0);
               __builtin_prefetch (&f1_layer[i+3].U, 0, 0);
               __builtin_prefetch (&f1_layer[i+4].W, 1, 1);
               __builtin_prefetch (&f1_layer[i+4].I[cp], 0, 0);
               __builtin_prefetch (&f1_layer[i+4].U, 0, 0);
	  }
	  else if (i==tope-1){
     		__builtin_prefetch (&a, 0, 0);
               __builtin_prefetch (&f1_layer[i].W, 1, 0);
               __builtin_prefetch (&f1_layer[i].I[cp], 0, 0);
               __builtin_prefetch (&f1_layer[i].U, 0, 0);
          
	  }*/
          f1_layer[i].W = f1_layer[i].I[cp] + a*(f1_layer[i].U);
          tnorm_aux += f1_layer[i].W * f1_layer[i].W;
      }
//    __builtin_prefetch (&tnorm_aux, 0, 0);
     if (tope==2500){
	tnorm1=tnorm_aux;}
     else if (tope==5000){
        tnorm2=tnorm_aux;}
...
void* floop2 (void* arg){
 	int tope= *(int*)arg;
      for (int i=tope-2500; i<tope;i++){
	  /*if (i%5==0){
	       __builtin_prefetch (&tnorm, 0, 3);
               __builtin_prefetch (&f1_layer[i].W, 0, 0);
               __builtin_prefetch (&f1_layer[i].X, 1, 0);
               __builtin_prefetch (&f1_layer[i+1].W, 0, 0);
               __builtin_prefetch (&f1_layer[i+1].X, 1, 0);
               __builtin_prefetch (&f1_layer[i+2].W, 0, 0);
               __builtin_prefetch (&f1_layer[i+2].X, 1, 0);
               __builtin_prefetch (&f1_layer[i+3].W, 0, 0);
               __builtin_prefetch (&f1_layer[i+3].X, 1, 0);
               __builtin_prefetch (&f1_layer[i+4].W, 0, 0);
               __builtin_prefetch (&f1_layer[i+4].X, 1, 0);
	  }
	  else if (i==tope-1){
	       __builtin_prefetch (&tnorm, 0, 0);
	  }*/
          f1_layer[i].X = f1_layer[i].W/tnorm;
      }

      loop2.commit();
}
void* floop3 (void* arg){
      int tope= *(int*)arg;
      double tnorm_aux=0;
      for (int i=tope-2500; i<tope;i++)
      {
  	...
  	/*if (i%5==0){
	       __builtin_prefetch (&xr, 1, 2);
	       __builtin_prefetch (&qr, 1, 2);
	       __builtin_prefetch (&theta, 0, 2);
	       __builtin_prefetch (&b, 0, 2);

               __builtin_prefetch (&f1_layer[i].X, 0, 1);
               __builtin_prefetch (&f1_layer[i].Q, 0, 1);
               __builtin_prefetch (&f1_layer[i].V, 1, 1);
               __builtin_prefetch (&tnorm_aux, 1, 0);
	  }
	  else if (i==tope-1){
     		__builtin_prefetch (&xr, 1, 0);
	       __builtin_prefetch (&qr, 1, 0);
           ...
           tnorm_aux += f1_layer[i].V * f1_layer[i].V;
      }
     if (tope==2500){
	tnorm1=tnorm_aux;}
     else if (tope==5000){
...
void* floop4 (void* arg){
     int tope= *(int*)arg;
      for (int i=tope-2500; i<tope;i++){
	/*if (i%5==0){
               __builtin_prefetch (&tnorm, 0, 3);
               __builtin_prefetch (&f1_layer[i].U, 1, 0);
               __builtin_prefetch (&f1_layer[i].V, 0, 0);
               __builtin_prefetch (&f1_layer[i+1].U, 1, 0);
               __builtin_prefetch (&f1_layer[i+1].V, 0, 0);
               __builtin_prefetch (&f1_layer[i+2].U, 1, 0);
               __builtin_prefetch (&f1_layer[i+2].V, 0, 0);
               __builtin_prefetch (&f1_layer[i+3].U, 1, 0);
               __builtin_prefetch (&f1_layer[i+3].V, 0, 0);
               __builtin_prefetch (&f1_layer[i+4].U, 1, 0);
               __builtin_prefetch (&f1_layer[i+4].V, 0, 0);
	  }
	else if (i==tope-1){
               __builtin_prefetch (&tnorm, 0, 0);
	}*/
          f1_layer[i].U = f1_layer[i].V/tnorm;
      }
...
void* floop5 (void* arg){
     int tope= *(int*)arg;
      ...
      for (int i=tope-2500; i<tope;i++)
      {   
  //      __builtin_prefetch (&f1_layer[i].P, 1, 2);
        tsum_aux = 0;
        ttemp_aux= f1_layer[i].P;

        for (int tj=cp;tj<numf2s;tj++)
        {
...
//	__builtin_prefetch (&f1_layer[i].U, 0, 0);
        f1_layer[i].P = f1_layer[i].U + tsum_aux;

//        tnorm_aux += f1_layer[ti].P * f1_layer[i].P;
        if (ttemp_aux != f1_layer[i].P)
           tresult=0;
     }
     /*if (tope==5000){
	tnorm1=tnorm_aux;}
...
void* floop6 (void* arg){
     int tope= *(int*)arg;
      for (int i=tope-2500; i<tope;i++){
/*	    if (i%5==0){
		__builtin_prefetch (&f1_layer[i].P, 0, 0);
		__builtin_prefetch (&f1_layer[i].Q, 1, 0);
		__builtin_prefetch (&f1_layer[i+1].P, 0, 0);
		__builtin_prefetch (&f1_layer[i+1].Q, 1, 0);
		__builtin_prefetch (&f1_layer[i+2].P, 0, 0);
		__builtin_prefetch (&f1_layer[i+2].Q, 1, 0);
		__builtin_prefetch (&f1_layer[i+3].P, 0, 0);
		__builtin_prefetch (&f1_layer[i+3].Q, 1, 0);
		__builtin_prefetch (&f1_layer[i+4].P, 0, 0);
		__builtin_prefetch (&f1_layer[i+4].Q, 1, 0);
	    }*/
...
{ int j,matched,f1res,mt;
  ...
  input_vars.push_back((void*)&input_a); //Input for first iteration*/
  ...
  { 
	...
	input_functions.push_back(floop1); 

      /* Compute F1 layer - W values */
      loop1.run (input_functions, input_vars);
      loop1.append(floop1,(void*)&input_b);
      loop1.append(floop1,(void*)&input_c);
      loop1.append(floop1,(void*)&input_d);
      loop1.commit();
      loop1.get_results();
      tnorm= tnorm1+ tnorm2+tnorm3+tnorm4;
      tnorm =  sqrt((double)tnorm);

      
      /* Compute F1 layer - X values */
      input_functions.clear();
      input_functions.push_back(floop2); 
      loop2.run (input_functions, input_vars);
      loop2.append(floop2,(void*)&input_b);
      loop2.append(floop2,(void*)&input_c);
      loop2.append(floop2,(void*)&input_d);
      loop2.commit();
      loop2.get_results();

      /* Compute F1 layer - V values */
      input_functions.clear();
      input_functions.push_back(floop3); 
      loop3.run (input_functions, input_vars);
      loop3.append(floop3,(void*)&input_b);
      loop3.append(floop3,(void*)&input_c);
      loop3.append(floop3,(void*)&input_d);
      loop3.commit();
      loop3.get_results();
      tnorm= tnorm1+ tnorm2+tnorm3+tnorm4;
      tnorm = sqrt((double) tnorm);
      ...
      input_functions.clear();
      input_functions.push_back(floop4); 
      loop4.run (input_functions, input_vars);
      loop4.append(floop4,(void*)&input_b);
      loop4.append(floop4,(void*)&input_c);
      loop4.append(floop4,(void*)&input_d);
      loop4.commit();
      loop4.get_results();
   ...
   /*   tnorm =0;
      tresult = 1;
      input_functions.clear();
      input_functions.push_back(floop5);
      loop5.run (input_functions, input_vars); 
      loop5.append(floop5,(void*)&input_b);
      loop5.append(floop5,(void*)&input_c);
      loop5.append(floop5,(void*)&input_d);
      loop5.commit();
      loop5.get_results();
...
/*      tnorm= tnorm1+ tnorm2;
      ...
      /* Compute F1 - Q values */
      input_functions.clear();
      input_functions.push_back(floop6);
      loop6.run (input_functions, input_vars); 
      loop6.append(floop6,(void*)&input_b);
      loop6.append(floop6,(void*)&input_c);
      loop6.append(floop6,(void*)&input_d);
      loop6.commit();
      loop6.get_results();
