...
long i,j;
double tnorm1, tnorm2, tnorm3, tnorm4;
...
/*parallel loops*/
parallel_loop_in_batches loop1, loop2, loop3, loop4;
...
{ int j,lines,k;
  ...
  double y1,y2;
   ...
   fscanf(fp,"%i",&lines);
   for (j=0;j<lines;j++)
   {   fscanf(fp,"%i %f %i %f",&x1,(float*)&y1,&x2,(float*)&y2);
       for (k=x1;k<=x2;k++)
...
{ int i;
  ...
  for(i=0;i<numf1s;i++)
     fscanf(fp,"%f",(float*)&f1_layer[i].I[cp]);
}
...
{ int j,matched,f1res,mt;
  ...
  input_vars.push_back(input); //Input for first iteration
...
//      tnorm2 = 0;
      ...
      //loop1.append(floop1,(void*)5000);
      //loop1.append(floop1,(void*)7500);
      //loop1.append(floop1,(void*)10000);
    //  loop1.commit();
      tnorm = 0;
//      loop1.get_results();
