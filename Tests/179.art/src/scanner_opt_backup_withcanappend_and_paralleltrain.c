...
int highx[20][2], highy[20][2];
double highest_confidence[20][2];
int set_high[20][2];
int numIt=0;
...
int winner[20];
int resonant[20];
int numf1s,numf2s, cp,numpatterns;

/*Variables for parallel execution*/
parallel_loop_in_batches scan_recognize_loop, train_match_loop;
bool hasRead[20];
bool canAppend[20];
bool didPass[20];
pthread_mutex_t train_match_mutex;
int sx, ex, st;
int train_matched;
int numTrainIt;
...
void find_match(int g)
{ int i;
   winner[g] = 0;
   for (i=0;i<numf2s;i++)
       if (Y[g][i].y > Y[g][winner[g]].y)
              winner[g]=i;
}
...
{   int j;
   ...
   for (j=0;j<numf1s;j++)
   {
      norm += f1_layer[0][j].P * f1_layer[0][j].P;
   }
   ...
   for (j=0;j<numf1s;j++)
   {
      temp_sum = f1_layer[0][j].U * f1_layer[0][j].U;
      norm += temp_sum;
   }
   norm = sqrt((double) norm);
   sum += norm;
   for (j=0;j<numf1s;j++){
      f1_layer[0][j].R =  (f1_layer[0][j].U+ c * f1_layer[0][j].P)/sum;
   }
   norm = 0;
   for (j=0;j<numf1s;j++)
      norm += f1_layer[0][j].R * f1_layer[0][j].R;
   norm = sqrt((double) norm);
#ifdef DEBUG
   if (DB2 && (winner[0]==0)) printf("%5.3f",norm);
#endif
return(norm);
}


double simtest2(int g)
{   int j;
   ...
   for (j=0;j<numf1s;j++)
   {
      su += f1_layer[g][j].U;
      sp += f1_layer[g][j].P;
      su2 += f1_layer[g][j].U * f1_layer[g][j].U;
      sp2 += f1_layer[g][j].P * f1_layer[g][j].P;
      sup += f1_layer[g][j].U * f1_layer[g][j].P;
   }
   ...
   if (DB2 && (winner[g]==0)) printf("%8.4f",r);
#endif
//   printf (" r:%f ", r);
   return (r);
}
...
{ int i,j,k;
  ...
  int good_count;
	...
	for (j=0;j<numf1s;j++)
	{    temp = tds[j][i];
             tds[j][i] += g(i)*(f1_layer[0][j].P - tds[j][i])*delta_t; 
	...
	for (j=0;j<numf1s;j++)
	{   temp = bus[j][i];
            bus[j][i] += g(i) * (f1_layer[0][j].P - bus[j][i]) * delta_t;
	    if ((fabs(temp - bus[j][i]) <= er) && resonant[0])
            {
#ifdef DEBUG
                 good_count++;
#endif
...
void init_net()
{
   ...
   for(int g=0; g<20; g++){
   f1_layer[g] = (f1_neuron *)malloc(numf1s * sizeof (f1_neuron)); 
   if (f1_layer[g]==NULL)
   {
   ...
   for (i=0;i<numf1s;i++)
   {
    for (int g=0; g<20; g++){
     ...
     f1_layer[g][i].I = (double *)malloc(2*sizeof(double));
     if (f1_layer[g][i].I==NULL)
     {
       fprintf(stderr,"malloc error in init_net\n");
       exit(1);
     }
   ...
   for(int g=0; g<20; g++){
   Y[g] = (xyz *)malloc(numf2s*sizeof(xyz));
   if (Y[g]==NULL)
   {
     fprintf(stderr,"Malloc error for Y\n");
     exit(1);
   }
   }  
}

void reset_nodes(int g)
{ int i;
...
void print_f12(int g)
{ int j;
     printf("\n\n");
     for(j=0;j<numf2s;j++)
         printf(" j = %i  Y= %9.7f\n",j,Y[g][j].y);
}
void* f_train_match_loop (void* arg){
      int j= (int)arg;
      int ti,tj,tresult, f1res;
      double tnorm;
      ...
      {
          f1_layer[0][ti].W = f1_layer[0][ti].I[cp] + a*(f1_layer[0][ti].U);
          tnorm += f1_layer[0][ti].W * f1_layer[0][ti].W;
      }
      tnorm =  sqrt((double)tnorm);
      /* Compute F1 layer - X values */

      for (tj=0;tj<numf1s;tj++)
            f1_layer[0][tj].X = f1_layer[0][tj].W/tnorm;
      ...
      for (ti=0;ti<numf1s;ti++)
      {
           if (f1_layer[0][ti].X < theta)
                  xr = 0;
           else
                  xr = f1_layer[0][ti].X; 
           if (f1_layer[0][ti].Q < theta)
                  qr = 0;
           else
                  qr = f1_layer[0][ti].Q;
           f1_layer[0][ti].V = xr + b*qr;
           tnorm += f1_layer[0][ti].V * f1_layer[0][ti].V;
      }

      /* Compute F1 layer - U values */
      tnorm = sqrt((double) tnorm);
      for (tj=0;tj<numf1s;tj++)
            f1_layer[0][tj].U = f1_layer[0][tj].V/tnorm;
      ...
      {   
        tsum = 0;
        ttemp = f1_layer[0][ti].P;

        for (tj=cp;tj<numf2s;tj++)
        {
           if ((tj == winner[0])&&(Y[0][tj].y > 0))
                 tsum += tds[ti][tj] * d;
        }

        f1_layer[0][ti].P = f1_layer[0][ti].U + tsum;

        tnorm += f1_layer[0][ti].P * f1_layer[0][ti].P;

        if (ttemp != f1_layer[0][ti].P)
           tresult=0;
      }
      f1res = tresult;
      
//      pthread_mutex_lock(&train_match_mutex);
      if (j!=8 && f1res){
	j++;
        numTrainIt++;
	train_match_loop.append (f_train_match_loop, (void*)j);
      }
      /* Compute F1 - Q values */

      tnorm = sqrt((double) tnorm);
      for (tj=0;tj<numf1s;tj++)
            f1_layer[0][tj].Q = f1_layer[0][tj].P;
      ...
      {    
        ...
        if ( !Y[0][tj].reset )
        for (ti=0;ti<numf1s;ti++)
           Y[0][tj].y += f1_layer[0][ti].P * bus[ti][tj];
      }

      /* Find match */
      winner[0] = 0;
      for (ti=cp;ti<numf2s;ti++)
      {
       if (Y[0][ti].y > Y[0][winner[0]].y)
          winner[0] =ti;
      }
  //    pthread_mutex_unlock(&train_match_mutex);

      train_match_loop.commit();
}
...
{ int mt, matched;
  ...
  input_functions.push_back(f_train_match_loop);
  vector <void*> input_vars;
  input_vars.push_back((void*)0); //Podria ser sumandole (8*stride).
  while (!matched)
  { 
    numTrainIt=0;
    train_match_loop.run(input_functions, input_vars);
    train_match_loop.commit();
    train_match_loop.get_results();
...
fprintf(stdout,"rho %e\n",match_confidence);
    ...
    {
#ifdef DEBUG
       if (DB2 && (winner[0]==0)) printf("#%i",winner[0]);
#endif
       weightadj();
       matched = 1;
    }
    ...
    {    Y[0][winner[0]].y = 0;
         Y[0][winner[0]].reset = 1;
#ifdef DEBUG
         if (DB1) printf("#%iN",winner[0]);
#endif
         matchtest=0;
...
{ int j,matched,f1res,mt;
  ...
  { 
    ...
    for (j=0;j<9 && !f1res ;j++)
    {
      ...
      for (ti=0;ti<numf1s;ti++)
      {
          f1_layer[g][ti].W = f1_layer[g][ti].I[0] + a*(f1_layer[g][ti].U);
          tnorm += f1_layer[g][ti].W * f1_layer[g][ti].W;
      }
      tnorm =  sqrt((double)tnorm);
      /* Compute F1 layer - X values */

      for (tj=0;tj<numf1s;tj++)
            f1_layer[g][tj].X = f1_layer[g][tj].W/tnorm;
      ...
      for (ti=0;ti<numf1s;ti++)
      {
           if (f1_layer[g][ti].X < theta)
                  xr = 0;
           else
                  xr = f1_layer[g][ti].X; 
           if (f1_layer[g][ti].Q < theta)
                  qr = 0;
           else
                  qr = f1_layer[g][ti].Q;
           f1_layer[g][ti].V = xr + b*qr;
           tnorm += f1_layer[g][ti].V * f1_layer[g][ti].V;
      }

      /* Compute F1 layer - U values */
      tnorm = sqrt((double) tnorm);
      for (tj=0;tj<numf1s;tj++)
            f1_layer[g][tj].U = f1_layer[g][tj].V/tnorm;
      ...
      {   
        tsum = 0;
        ttemp = f1_layer[g][ti].P;

        for (tj=0;tj<numf2s;tj++)
        {
           if ((tj == winner[g])&&(Y[g][tj].y > 0))
                 tsum += tds[ti][tj] * d;
        }

        f1_layer[g][ti].P = f1_layer[g][ti].U + tsum;

        tnorm += f1_layer[g][ti].P * f1_layer[g][ti].P;

        if (ttemp != f1_layer[g][ti].P)
           tresult=0;
      }
      f1res = tresult;
      ...
      tnorm = sqrt((double) tnorm);
      for (tj=0;tj<numf1s;tj++)
            f1_layer[g][tj].Q = f1_layer[g][tj].P;
      ...
      {    
        ...
        if ( !Y[g][tj].reset )
        for (ti=0;ti<numf1s;ti++)
           Y[g][tj].y += f1_layer[g][ti].P * bus[ti][tj];
      }
...
#endif
    match_confidence=simtest2(g);
 //   cout<<"winner: "<<winner[g]<<" g: "<<g<<" j: "<<j<<endl;
    if ((match_confidence) > rho)
    {
       /* If the winner is not the default F2 neuron (the highest one)
        * we have a match.
        */
       if (winner[g]!=numf2s-1)
       {
           pass=true;
           fprintf(stdout,"F2 neuron %d passes vigilance with a value of %0.4f\n",winner[g],match_confidence);
           print_f12(g);
           if (match_confidence > highest_confidence[g][winner[g]])
           {
             highest_confidence[g][winner[g]] = match_confidence;
             set_high[g][winner[g]] = TRUE;
           }
       }
       matched = 1;
    }
    ...
    {    Y[g][winner[g]].y = 0;
         Y[g][winner[g]].reset = 1;
#ifdef DEBUG
         if (DB1) printf("#%i No",winner[g]);
#endif
         matchtest=0;
...
void* f_scan_recognize_loop (void* arg){
       int j=(int)arg;
       int it=numIt;
       long k;
       int m, n;
       bool pass;
       hasRead[it]=true;
       for (int i=sx;i<ex;i+=st)
       {  
         k=0;
         for (m=j;m<(lheight+j);m++){
           for (n=i;n<(lwidth+i);n++){
               f1_layer[it][k++].I[0] = cimage[m][n];
	       }
         }
         pass=match(it);
         if (pass)
...
#endif
	    didPass[it]=true;
            if (set_high[it][0]==TRUE)
            {
              highx[it][0] = i;
              highy[it][0] = j;
              set_high[it][0] = FALSE;
            }
            if (set_high[it][1]==TRUE) 
            {
              highx[it][1] = i;
              highy[it][1] = j;
              set_high[it][1] = FALSE;
...
{ 
  ...
  st=stride;
  script_vector input_functions;
  input_functions.push_back(f_scan_recognize_loop);
  vector <void*> input_vars;
  input_vars.push_back((void*)starty); //Podria ser sumandole (8*stride).
  numIt=0; //num it podria arrancar de 8
  bool good2go;
  for (int r=0; r<20; r++){
     hasRead[r]=false;
     canAppend[r]=false;
  ...
  int lastPassed=-1;
  scan_recognize_loop.run (input_functions, input_vars);
    for (j=starty+stride;j<endy;j=j+stride ){ //Podria ser con 9 stride
            if (!hasRead[numIt]){ 
		j=j-stride;
	    }
            else {
	     if (lastPassed!=-1){
		      if (canAppend[lastPassed+1]){
			 if (didPass[lastPassed+1]){
				lastPassed++;
				if (lastPassed==numIt){
					numIt++;
                        		scan_recognize_loop.append(f_scan_recognize_loop,(void*)j);
		//			cout<<numIt<<" appended"<<endl;
				}
				else{
					j=j-stride;
				}
			...
			j=j-stride;
             ...
             else if (numIt>=3){ //Podria ser 11
                  good2go=true;
		 if (canAppend[numIt-3]){
              	      if (didPass[numIt-3] && lastPassed<numIt-3){
				lastPassed=numIt-3;
				good2go=false;
				j=j-stride;
		      }
	             canAppend[numIt-3]=false;
		  }
		  else if (canAppend[numIt-2]){
              	      if (didPass[numIt-2] && lastPassed<numIt-2){
				lastPassed=numIt-2;
				good2go=false;
				j=j-stride;
		      }
	             canAppend[numIt-2]=false;
		  }
		  else if (canAppend[numIt-1]){
              	      if (didPass[numIt-1] && lastPassed<numIt-1){
				lastPassed=numIt-1;
				good2go=false;
				j=j-stride;
		      }
	             canAppend[numIt-1]=false;
		  }
                  else if (canAppend[numIt]){
                      if (didPass[numIt] && lastPassed<numIt){
				lastPassed=numIt;
		      }
		     canAppend[numIt]=false;
		     if (lastPassed==19){
			 good2go=false;
			 j=endy;
		     }
    ...
    scan_recognize_loop.commit();
    scan_recognize_loop.get_results();
}
...
{  int k;
   ...
   pthread_mutex_init(&train_match_mutex, NULL);
...
//       }
   ...
   for (int g=0; g<20; g++){
   highest_confidence[g][0] = 0.0;
   highest_confidence[g][1] = 0.0;
   highx[g][0] = 0;
   highx[g][1] = 0;
   highy[g][0] = 0;
   highy[g][1] = 0;
   ...
   int hx0, hx1, hy0, hy1; 
   double hc0;
   hc0=highest_confidence[0][0];
   double hc1;
   hc1=highest_confidence[0][1];
   hx0=highx[0][0];
   hx1=highx[0][1];
   hy0=highy[0][0];
   hy1=highy[0][1];
   for (int g=1; g<20; g++){
     if (highest_confidence[g][0]>hc0){
          hc0=highest_confidence[g][0];
	  hx0=highx[g][0];
	  hy0=highy[g][0];
     }
     if (highest_confidence[g][1]>hc1){
          hc1=highest_confidence[g][1];
	  hx1=highx[g][1];
          hy1=highy[g][1];
     }
   }
