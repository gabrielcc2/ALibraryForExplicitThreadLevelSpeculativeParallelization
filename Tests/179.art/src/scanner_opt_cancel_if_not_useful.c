...
parallel_loop_in_batches loop1;
bool hasRead;
...
//    cout<<"winner: "<<winner[g]<<" g: "<<g<<" j: "<<j<<endl;
    if ((match_confidence) > rho)
    {
       ...
       {
	   if (cancellationTop==0){
		if (g==14){
//			cout<<"Proceding to cancel..."<<endl;
			for (int x=0; x<20; x++){
				if (x!=14){
					loop1.cancel_iteration(x);
				}
			}
			cancellationTop=g;
		}
		else {
//			cout<<"Proceding to cancel 2..."<<endl;
			for (int x=0; x<20; x++){
				if (x<8 || x>12){
					loop1.cancel_iteration(x);
				}
...
{ 
  ...
  input_functions.push_back(f_loop1);
  vector <void*> input_vars;
  input_vars.push_back((void*)starty);
  hasRead=false;
  cancellationTop=0;
  numIt=0;
  loop1.run (input_functions, input_vars);
    for (j=starty+stride;j<endy;j=j+stride ){
            if (!hasRead){ 
		j=j-stride;
	    }
            else {
...
//       }
 ...
 //  winner[g]=winner[0];
   ...
   /*Extra: Copy results from train_match on [0] to others
   for (int h=0; h<numf1s; h++){
	for (int f=1; f<20; f++){
      f1_layer[f][h].W=f1_layer[0][h].W;
      f1_layer[f][h].X=f1_layer[0][h].X;
      f1_layer[f][h].V=f1_layer[0][h].V;      
      f1_layer[f][h].U=f1_layer[0][h].U;
      f1_layer[f][h].P=f1_layer[0][h].P;
      f1_layer[f][h].Q=f1_layer[0][h].Q;
      f1_layer[f][h].R=f1_layer[0][h].R;}
   }*/
