...
/*Variables for parallel run*/
parallel_loop_in_batches match_loop;
int f1res;
int num_it;
pthread_mutex_t p_section_mutex;

/*Variables for prefetching*/
ht_speculator htspec;
...
void* match_loop_f (void* arg){
      ...
      /*LOOP 1*/
      /*Prefetch analysis: a has to be in memory during the whole run. f1_layer[ti] can have one miss per run, and should be flushed from the cache asap
      */
      htspec.speculate(prefetch_loop1, (void*)&pf_input);
      tnorm = 0; 
      ...
      /*LOOP 2*/
      /*Prefetch analysis: tnorm has to be in memory during the whole run. f1_layer[ti] can have one miss per run, and should be flushed from the cache asap
      What is pointed by I* should not get on the cache.
      */

      htspec.speculate(prefetch_loop2, (void*)&pf_input);
      ...
      /*LOOP 3*/
      /*Prefetch analysis: theta, xr, b, qr have to be in memory during the whole run. 
      f1_layer[ti] can have one miss per run, and should be flushed from the cache asap. 
      What is pointed by I* should not get on the cache.
      */

      htspec.speculate(prefetch_loop3, (void*)&pf_input);
      tnorm =0;
      for (ti=0;ti<numf1s;ti++)
      {
      ...
      /*LOOP 4*/
      /*Prefetch analysis: tnorm has to be in memory during the whole run. 
      f1_layer[ti] can have one miss per run, and should be flushed from the cache asap. 
      What is pointed by I* should not get on the cache.*/
      /* Compute F1 layer - U values */
      htspec.speculate(prefetch_loop4, (void*)&pf_input);
      tnorm = sqrt((double) tnorm);
      for (tj=0;tj<numf1s;tj++){
      ...
      pthread_mutex_lock(&p_section_mutex);

      htspec.speculate(prefetch_loop5, (void*)&pf_input);
      /*LOOP 5*/
      /*Prefetch analysis: tsum, ttemp, cp, numf2s have to be in memory during the whole run. 
      f1_layer[ti] can have one miss per run, and should be flushed from the cache asap. 
      What is pointed by I* should not get on the cache.
      The if conditions will not be considered for the prefetch.*/
      ...
      if (j<8 && !f1res){
          num_it++;
          match_loop.append(match_loop_f, (void*)&num_it);
      }


      /*LOOP 6*/
      /* Compute F1 - Q values */
      htspec.speculate(prefetch_loop6, (void*)&pf_input);
      tnorm = sqrt((double) tnorm);
      for (tj=0;tj<numf1s;tj++)
     ...
     pthread_mutex_unlock(&p_section_mutex);
     match_loop.commit();
}
...
{ int j,matched, mt;
  ...
  input_functions.push_back(match_loop_f);
  vector <void*> input_vars;
  input_vars.push_back((void*)&j);
...
//    for (j=0;j<9 && !f1res ;j++)
  //  {
    match_loop.run(input_functions, input_vars);
    match_loop.commit();
    match_loop.get_results();
    j=num_it;
...
{  int k;
  ...
  pthread_mutex_init(&p_section_mutex, NULL);
