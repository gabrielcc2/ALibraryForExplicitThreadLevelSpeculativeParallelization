...
int highx[4][2], highy[4][2];
double highest_confidence[4][2];
int set_high[4][2];
int numIt=0;
...
void* f_loop1 (void* arg){
       ...
       {  
         ...
         if (pass)
         {
            ...
            printf(" at X= %d Y = %d\n",i,j);
#endif
            if (set_high[it2][0]==TRUE)
            {
              highx[it2][0] = i;
              highy[it2][0] = j;
              set_high[it2][0] = FALSE;
            }
            if (set_high[it2][1]==TRUE) 
            {
              highx[it2][1] = i;
              highy[it2][1] = j;
              set_high[it2][1] = FALSE;
            }
         }
#ifdef DEBUG
         else if (DB3)
...
{ 
  ...
  loop1.run (input_functions, input_vars);
    for (j=starty+(5*stride);j<endy;j+=(5*stride) ){
            if (!hasRead){ 
		j-=(5*stride);
	    }
            else {
