...
* returns
*/
int noel (int p1, int p2, float d, float dm, float dh,float km, float kh)
{
...
/* check to see if a noelt is already defined */
	...
	{
		if( new_p == NULL) break;
		if( (new_p->atom1 == ap1 && new_p->atom2 == ap2) ||
                    (new_p->atom1 == ap2 && new_p->atom2 == ap1) )
		{
		new_p->d = d; new_p->dm = dm; new_p->dh = dh;  
		new_p->km = km; new_p->kh = kh; return 1;
		}
		if( new_p == new_p->next) break;
		new_p = (NOEL*) new_p->next;
	}
	}
	if( ( new_p = (NOEL*) malloc( NOELLONG ) ) == NULL)
	{
	return 0;
	}
	/* initialize the pointers */
	if( noel_first == NULL) noel_first = new_p;
	if( noel_last == NULL) noel_last = new_p;
	new_p -> atom1 = ap1;
	new_p -> atom2 = ap2;
		new_p->d = d; new_p->dm = dm; new_p->dh = dh;  
		new_p->km = km; new_p->kh = kh; 
	new_p -> next = new_p;
	noel_last -> next = new_p;
	noel_last = new_p;
	return 1;
	}
...
* V is the potential */
int v_noel(float *V, float lambda)
{
	NOEL *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_noel(float *V,float lambda, int ilow, int ihigh, 	FILE *op)
{
	NOEL *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int v_ho_noel(float *V, float lambda)
{
	NOEL *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
