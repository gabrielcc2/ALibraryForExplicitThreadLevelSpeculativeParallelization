...
void gsdg(int (*vfs[])(float*, float), int nfs, int niter, int low_serial, int high_serial)
{
ATOM *ap,*bp,*a_next(int);
int numatm , a_number();
int iter, i,j,k;
float svec[3],rvec[3]; /* storage for search vectors */
float x,y,z;
float gsdg_line_search(float[3], float*, ATOM*);
int v_bond(float*, float),v_angle(float*, float),u_v_nonbon(float*, float),v_noel(float*, float);
int v_mmbond(float*, float),v_mmangle(float*, float),v_c_angle(float*, float),v_nonbon(float*, float);
int v_hybrid(float*, float);
...
float gsdg_line_search(float vect[3], float *step, ATOM *who)
{
float val;
float vt,lam;
int i,j;
...
float gsdg_dgeom(float[3], float, ATOM*);
...
float gsdg_dgeom(float vect[3], float lam, ATOM* who)
{
int numatm,a_number();
int i;
float x,y,z;
...
int v_trace(float *V, float lambda)
{
	int numatm,i,a_number();
	ATOM *ap,*a_next(int);
	float xc,yc,zc;
	float xt,yt,zt;
	float l_trace,get_f_variable(char*);
