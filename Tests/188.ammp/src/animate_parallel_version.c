...
*
*/
int tpac(int (*[])(float), int, int, float,float);
...
/*  move the velocity vector into the displacment slot */
	ke = 0.; //VAR PRIVADA
	imax = a_number(); //Var compartida
	ap = a_next(-1); //Var compartida
	bp = ap; //VAR PRIVADA
	for( i=0; i< imax; i++,ap = bp)
	{
	bp = a_next(1); //Var compartida
	ke += ap->mass*(
