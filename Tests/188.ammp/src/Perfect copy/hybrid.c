...
* returns
*/
int hybrid(int p1,int p2,int p3, int p4, float fk, float off)
{
	HYBRID  *new_p;
	ATOM  *ap1, *ap2, *ap3, *ap4, *a_m_serial(int);
	char line[80];
/* get the atom pointers for the two serial numbers */
	...
	if( ( new_p = (HYBRID*) malloc((unsigned int) HLONG ) ) == NULL)
	{
	return 0;
	}
	/* initialize the pointers */
	if( hybrid_first == NULL) hybrid_first = new_p;
	if( hybrid_last == NULL) hybrid_last = new_p;
	new_p -> atom1 = ap1;
	new_p -> atom2 = ap2;
	new_p -> atom3 = ap3;
	new_p -> atom4 = ap4;
...
* V is the potential */
int v_hybrid( float *V, float lambda)
{
	HYBRID  *bp;
/* difference vectors */
...
void get_hybrid(ATOM  *a1, ATOM *bonded[], int mbond, int *inbond)
{
	...
	*inbond = 0;
	...
	if( mine == (HYBRID*) mine->next) return;
...
void dump_hybrids( FILE *where )
{
	...
	b = hybrid_first;
	if( b == NULL ) return;
	while( ((HYBRID*)b->next != b)  )
	{
	if( (HYBRID*)b->next == NULL) return;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_hybrid(float *V, float lambda, int ilow,int ihigh, FILE *op)
{
	HYBRID  *bp;
/* difference vectors */
	float x1,y1,z1,x2,y2,z2,x3,y3,z3;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int v_ho_hybrid(float *V, float lambda)
{
	HYBRID  *bp;
/* difference vectors */
	float x1,y1,z1,x2,y2,z2,x3,y3,z3;
