...
/* in order to have the  array matrix be of any length it must be passed as
   ...
   ilead is the row size of the array to which matrix points */
	int mom_solve( )
//float (*matrix)[], float (*vector)[], int irow, int ilead 
	{
/*	 float quotient;
	int i,j,k;
	int  mpi,mpj,mpk;
...
void mom(FILE *op, float tq, int niter)
{
float (*v)[], (*Z)[],(*Zk)[];
float r,dx,dy,dz;
float dv,tv;
int i,j,row;
float mom_jab(float, float, float);

	if( in_mom_list == 0 ) return;
	if( niter <= 0 ) niter = 20; /* default number of trials */
	v = (float (*)[]) malloc( in_mom_list * sizeof(float)) ;
	Z = (float (*)[])malloc( in_mom_list*in_mom_list * sizeof(float)) ;
	Zk = (float (*)[]) malloc( in_mom_list*in_mom_list * sizeof(float)) ;
...
float mom_jab(float r, float j1, float j2 )
{
float a,b,b2,b3;
