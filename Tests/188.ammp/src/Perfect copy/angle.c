...
#define ALONG_ANGLE sizeof(ANGLE)
...
* returns
*/
int angle(int p1, int p2, int p3, float bl, float fk)
	{
	ANGLE *new_p;
	ATOM *ap1,*ap2,*ap3,*a_m_serial(int);
	char line[80];
	int i;
/* get the atom pointers for the two serial numbers */
	...
	if( ( new_p =  (ANGLE*) malloc( ALONG_ANGLE ) ) == NULL)
	{
	return 0;
	}
	/* initialize the pointers */
	if( angle_first == NULL) angle_first = new_p;
	if( angle_last == NULL) angle_last = new_p;
	new_p -> atom1 = ap1;
	new_p -> atom2 = ap2;
	new_p -> atom3 = ap3;
	new_p -> target = bl;
...
* V is the potential */
int v_angle(float *V, float lambda)
{
	ANGLE *bp;
...
void get_angle(ATOM *a1, ATOM *bonded[], int mbond, int *inbond)
{
	...
	*inbond = 0;
	...
	if( mine == (ANGLE*) mine->next) return; 
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int v_mmangle(float *V, float lambda)
{
	ANGLE *bp;
	float r,x1,y1,z1,x2,y2,z2;
	float dp;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_angle(float *V, float lambda, int ilow, int ihigh, FILE *op)
{
	ANGLE *bp;
	float r,x1,y1,z1,x2,y2,z2;
	float dp;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_mmangle(float *V, float lambda, int ilow, int ihigh, FILE *op)
{
	ANGLE *bp;
	float r,x1,y1,z1,x2,y2,z2;
	float dp;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int v_c_angle(float *V, float lambda)
{
	ANGLE *bp;
	float r,x1,y1,z1,x2,y2,z2;
	float dp;
...
int a_c_angle(float *V, float lambda, int ilow, int ihigh, FILE *op)
{
	ANGLE *bp;
	float r,x1,y1,z1,x2,y2,z2;
	float dp;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int v_ho_angle(float *V, float lambda)
{
	ANGLE *bp;
	float r,x1,y1,z1,x2,y2,z2;
	float dp;
	ATOM *a1,*a2,*a3;
	float hol,get_f_variable(char*),target;
