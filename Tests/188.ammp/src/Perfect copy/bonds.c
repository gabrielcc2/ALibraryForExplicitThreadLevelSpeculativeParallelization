...
* returns
*/
int bond(int p1, int p2, float bl, float fk)
	{
...
/* get the atom pointers for the two serial numbers */
	...
	if( ( new_p = (BOND*) malloc( BLONG ) ) == NULL)
	{
	return 0;
	}
	/* initialize the pointers */
	if( bond_first == NULL) bond_first = new_p;
	if( bond_last == NULL) bond_last = new_p;
	new_p -> atom1 = ap1;
	new_p -> atom2 = ap2;
	new_p -> length = bl;
	new_p -> k = fk;
...
* V is the potential */
int v_bond(float *V, float lambda)
{
	BOND *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int v_mmbond(float *V, float lambda)
{
	BOND *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_bond(float *V, float lambda, int ilow, int ihigh, FILE* op )
{
	BOND *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_mmbond(float *V, float lambda, int ilow, int ihigh, FILE *op)
{
	BOND *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
...
BOND *bp;
	...
	if( bp == (BOND*) bp->next) return NULL;
	bp = (BOND*) bp->next;
	}
	return NULL;
}
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int v_ho_bond(float *V, float lambda)
{
	BOND *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
