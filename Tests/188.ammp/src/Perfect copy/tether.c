...
* returns
*/
int tether(int p1, float fk , float x, float y, float z)
	{
...
/* check to see if it exists and update if so */
	...
	while(1)
	{
		if( new_p ->next == NULL) break;
		if( (ATOM*) new_p->atom1 == ap1)
		{
			new_p -> k = fk;
			new_p -> x = x;
			new_p -> y = y;
		...
		if( (TETHER*) new_p->next == new_p ) break;
		new_p = (TETHER*) new_p->next;
	}
	}	
	if( ( new_p = (TETHER*) malloc( tetherLONG ) ) == NULL)
	{
	return 0;
	}
	/* initialize the pointers */
	if( tether_first == NULL) tether_first = new_p;
	if( tether_last == NULL) tether_last = new_p;
	new_p -> atom1 = ap1;
	new_p -> k = fk;
	new_p -> x = x;
	new_p -> y = y;
	new_p -> z = z;
	new_p -> next = new_p;
	tether_last -> next = new_p;
	tether_last = new_p;
	return 1;
	}


int alltether(float fk)
{
	ATOM *ap1 ;
	ATOM *a_next(int);	
...
* V is the potential */
int v_tether(float *V, float lambda)
{
	TETHER *bp;
	float r,xt,yt,zt;
	ATOM *a1;
...
void dump_tethers( FILE *where )
{
	...
	b = tether_first;
	if( b == NULL ) return;
	while( ((TETHER*)b->next != b) )
	{
	if( (TETHER*)b->next == NULL) return;
	a1 = b->atom1; 
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_tether(float *V, float lambda, int ilow, int ihigh, FILE *op)
{
	TETHER *bp;
	float r,xt,yt,zt;
	float rms, rmax;
	int tried,imax;
	ATOM *a1;
	float bstrot(float [], float [], float [], float [], float [], float [], int, float[3][3], float[3]);
...
/*	int set_f_variable(); */
	...
	bp = tether_first;
       if( bp == NULL ) return 1;
	numatm = a_number();
	x = (float (*)[]) malloc( numatm*sizeof(float));
	y = (float (*)[]) malloc( numatm*sizeof(float));
	z = (float (*)[] )malloc( numatm*sizeof(float));
 	xx = (float (*)[]) malloc( numatm*sizeof(float));
	yy = (float (*)[]) malloc( numatm*sizeof(float));
	zz = (float (*)[]) malloc( numatm*sizeof(float));
...
float bstrot(float x[], float y[], float z[], float xx[], float yy[], float zz[], int na, float matrix[3][3], float delta [])
{
...
/* x */	
		...
		sx[1][2] = tz;
		sx[2][2] = ty;
		matmul( &sx[0][0],&tensor[0][0],&sq[0][0],3,3);
		cpyvec(&sq[0][0],&tensor[0][0],9);
		matmul( &sx[0][0],&matrix[0][0],&sq[0][0],3,3);
		cpyvec( &sq[0][0],&matrix[0][0],9);
/* y */	
		...
		sy[0][2] = tz;
		sy[2][2] = ty;
		matmul( &sy[0][0],&tensor[0][0],&sq[0][0],3,3);
		cpyvec(&sq[0][0],&tensor[0][0],9);
		matmul( &sy[0][0],&matrix[0][0],&sq[0][0],3,3);
		cpyvec( &sq[0][0],&matrix[0][0],9);
/* z */	
		...
		sz[1][0] = tz;
		sz[0][0] = ty;
		matmul( &sz[0][0],&tensor[0][0],&sq[0][0],3,3);
		cpyvec(&sq[0][0],&tensor[0][0],9);
		matmul(&sz[0][0],&matrix[0][0], &sq[0][0],3,3);
		cpyvec(&sq[0][0],&matrix[0][0],9);
...
void cpyvec(float orig[], float copy[], int n)
{
	int i;
	for( i=0; i< n; i++)
		copy[i] = orig[i];
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int v_ho_tether(float *V, float lambda)
{
	TETHER *bp;
	float r,xt,yt,zt;
	ATOM *a1;
        float hol, get_f_variable(char*);
