...
* returns
*/
int torsion(int p1, int p2, int p3, int p4, float fk, int n, float off)
	{
	TORSION *new_p;
...
/* get the atom pointers for the two serial numbers */
	...
	if( ( new_p = (TORSION*) malloc( TLONG ) ) == NULL)
	{
	return 0;
	}
	/* initialize the pointers */
	if( torsion_first == NULL) torsion_first = new_p;
	if( torsion_last == NULL) torsion_last = new_p;
	new_p -> atom1 = ap1;
	new_p -> atom2 = ap2;
	new_p -> atom3 = ap3;
	new_p -> atom4 = ap4;
...
* V is the potential */
int v_torsion(float *V, float lambda)
{
	TORSION *bp;
/* difference vectors */
...
SKIP:
	if( bp == (TORSION*) bp->next ) return 1;
	bp = (TORSION*) bp->next;
       }
}
/* f_torsion()
...
void get_torsion(ATOM* a1, ATOM* bonded[],int mbond,int *inbond)
{
	...
	*inbond = 0;
	...
	if( mine == (TORSION*) mine->next) return;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_torsion(float *V, float lambda, int ilow, int ihigh, FILE* op)
{
	TORSION *bp;
/* difference vectors */
...
float b1,b2,b3;
	...
	{ 
		...
		if( tp == (TORSION*) tp->next) return 0;
		tp = (TORSION*) tp->next;
	}
}
