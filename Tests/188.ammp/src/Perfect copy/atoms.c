...
* returns
*/
int atom(float x, float y, float z, int serial, float q, float a, float b, float mass, char* name )
	{
	int i;
	static int highest=-1,lowest=-1;
	ATOM *new_p, *a_m_serial(int);
	char *ctemp;
	new_p = NULL;
	if( highest >= serial && lowest <= serial) new_p = a_m_serial( serial);
	if( new_p == NULL)
	{
	if( ( new_p =  (ATOM*) malloc( ALONG ) ) == NULL)
	{
	return 0;
	}
	new_p ->dontuse = 0;
	new_p ->dontuse = 1;
	new_p ->excluded[0] = new_p;
	new_p -> active = 1;
	...
	if( first == NULL){ first = new_p;
	highest = serial; lowest = serial; }
	if( last == NULL) last = new_p;
	new_p ->x = x;
	new_p ->y = y;
	new_p ->z = z;
	new_p ->fx = 0.;
	...
	for( i=0; i< n; i++ )
     {
      ...
      if( ap == (ATOM*) ap->next)ap = first ;
      else ap = (ATOM*) ap->next;
     }
     return NULL;
}
...
ATOM *a_next(int flag)
{
	...
	if( flag <= 0){ ap = first; return ap;} 
      if( ap == (ATOM*) ap->next) return NULL;
      ap =(ATOM*) ap->next;
      return ap;
}
/* function a_f_zero()
...
* with a scale factor.
*/
int a_ftovx(float lambda, float lamold)
{
	ATOM *ap;
	ap = first;
	if( ap == NULL) return 0;
...
float a_max_f()
{
	...
	if( l2norm > l2max )
		l2max = l2norm;
	 if( ap == (ATOM*) ap->next) return l2max;
	 ap = (ATOM*) ap->next;
	}
}
/* function a_max_d() 
...
{
	ATOM *a,*ap,*a_next(int);
	int istailored;
	int get_i_variable(char*);
	int numatm,a_number();
	int i,j;
...
*/
int a_readvelocity(int serial, float vx,float vy, float vz)
{
	ATOM *ap,*a_m_serial(int);
