...
*/
parallel_loop_in_batches istep_loop;
pthread_mutex_t it_mutex;
int (**forces_global)(int);
int nforces_global;
float dtime_real_global,T_global;
...
*/
int verlet(int (*forces[])(int), int nforces, int nstep, float dtime)
{
...
*/
int pac(int (*forces[])(int), int nforces, int nstep, float dtime)
{
...
*
*/
int tpac(int (*[])(int), int, int, float,float);
void* f_istep_loop (void* arg){
...
/*  move the velocity vector into the displacment slot */
	ke = 0.; //VAR PRIVADA
	imax = a_number(); //VAR PRIVADA
	ap = a_next(-1); //VAR PRIVADA
	bp = ap; //VAR PRIVADA
	for( i=0; i< imax; i++,ap = bp)
	{
	bp = a_next(1);
...
/* scale the current velocities */
	dtime = dtime_real_global;
	if( Tke > 1.e-6)
	{	
	ke = sqrt(T_global/Tke); /* ke is the scaled shift value */
	dtime = dtime_real_global/ke;
/* 0.00002 is 2fs, this is near the limit so don't use it */
	if( dtime > 0.000020 ){
		tpac(forces_global,nforces_global,1,dtime_real_global*0.5,T_global); 
		tpac(forces_global,nforces_global,1,dtime_real_global*0.5,T_global); 
		goto SKIP;
			}
	ap = a_next(-1);
...
/*  find the force at the midpoint */
	a_f_zero();
	for( iforces=0;iforces<nforces_global; iforces++)
		(*forces_global[iforces])( dtime/2.);
/* update velocities */        
	imax = a_number();
	ap = a_next(-1);
	bp = ap;
...
int tpac(int (*forces[])(int), int nforces, int nstep, float dtime_real, float T)
{
  	forces_global=forces;
	nforces_global=nforces;
	dtime_real_global=dtime_real;
	T_global=T;
	...
	input_functions.push_back(f_istep_loop);

	pthread_mutex_init(&it_mutex, NULL);
	istep_loop.run(input_functions, input_vars);
        for( istep = 1; istep< nstep; istep++) //INTENTARE PARALELIZAR EL ISTEP LOOP!
	{
		istep_loop.append(f_istep_loop, (void*)istep);
	}
	istep_loop.commit();
	istep_loop.get_results();
	return 1;
}
...
*  ideally several cycles are good
*/
int pacpac(int (*forces[])(int),int nforces, int nstep, float dtime)
{
...
*  ideally several cycles are good
*/
int hpac(int (*forces[])(int), int (*poten[])(float), int nforces, int nstep, float dtime_real, float H)
{
	ATOM *ap,*bp,*a_next(int),*bonded[10];
	float ke,Tke;
	float alpha;
...
*  ideally several cycles are good
*/
int ppac(int (*forces[])(int), int nforces, int nstep, float dtime_real, float P)
{
	ATOM *ap,*bp,*a_next(int),*bonded[10];
	float p,Tp,R;
...
*  ideally several cycles are good
*/
int ptpac(int (*forces[])(int ), int nforces, int nstep, float dtime_real, float P, float T)
{
	ATOM *ap,*bp,*a_next(int),*bonded[10];
	float p,Tp,R;
	float Tk;
