...
int tgroup(int which, int context, int b1, int b2, int b3, int ntry, float base)
{
 /* context b1 b2 b3 are atom serial numbers */

TGROUP *tgp,*tgo;
ATOM *bonded[20],*a_m_serial(int);
...
void get_bond(ATOM*, ATOM*[], int, int*);
	...
	if( (tgp = (TGROUP*) malloc( tg_long)) == NULL)
...
found:
	...
	if( tgo == NULL ) tg_first = (TGROUP*) tgp->next;  /* delete the first link */
	free( tgp );
	return 1;
...
int tsearch(int t1, int t2, int t3, int t4, int t5, int t6, int t7, int t8)
{
	...
	TGROUP  *grouplist[8],*match_tgroup(int);
	int ngroup,i,j;
	int bestlist[8];
	float V;
	void tg_do_search(float*, int, int, int [], TGROUP* []);
	void tg_gen_con(TGROUP *[],int[], int), tg_init(TGROUP*[], int*,int),tg_apply(TGROUP*, int);
...
/* do the search  (recursive) */
	tg_do_search(&V, 0,ngroup, bestlist, grouplist);
/*
	for( i=0; i< ngroup; i++)
...
* grouplist 
*/
void tg_do_search(float *Vp, int igl, int ngl, int bl[], TGROUP *gl[])
{
...
if( igl > ngl) return;
	...
	{
	tg_do_search(&vl[i],igl+1,ngl,lbl, gl);
/* evaluate my energy */
/* int tg_nonbon( V,tgp ) */
...
*/
void tg_d_apply(TGROUP *tgp, float off)
{
...
* V is the potential */
int tg_nonbon(float *V, TGROUP *tgp)
{
	float r,r0,xt,yt,zt;
