...
*  when to stop
*/
int steep(int (*vfs[])(float*, float),int (*ffs[])(int),int nfs, int nstep, float toler)
{
	int i,j,ifs;
	float vb,vt,vto;
	float a_max_f(),lam;
...
*  when to stop
*/
int cngdel(int (*vfs[])(float*,float),int (*ffs[])(int),int nfs, int nstep, int nreset, float toler, int echo)
{
/* SPEC add function protos to reduce compiler warnings jh/9/21/99 */
int a_f_zero();
int a_ftodx(float,float);
	...
	float vb,vt,vto;
	float a_max_f(),lam;
	float linmin(int(*[])(float*,float),int,float), a_max_d();
	float a_pr_beta();
...
*/
float linmin(int (*ffs[])(float*, float), int nfs, float damp)
{
	int i,iter,jter,imin;
	float alpha[401],fval[401],dstep,step,stpmin,fvt,fmin,fold;
	int nostep,get_i_variable(char*);
