...
*/
#define ANSI 1
#define MAXTOKEN_EVAL 20 
#define TOKENLENGTH_EVAL 80 
...
int  itemp[MAXTOKEN_EVAL];  /* integers to read */
float ftemp[MAXTOKEN_EVAL]; /* floats to read */
int inbuff;     /* where am i in the buffer */
int inliteral ; /* inside of a literal string */
int eval(FILE*, FILE*, char *);
...
*/ 
  int  (*potentials[10])(float*, float);
  int (*forces[10])(float);
  int nused=(-1);
int eval(FILE *ip, FILE *op, char *line)
{
FILE *newfile,*fopen(const char*, const char*),*tmpfile();
char  token[MAXTOKEN_EVAL][TOKENLENGTH_EVAL],*ap, *ap1; 
	/* buffer and pointers for sscanf */
char   errmes[80];
int  itemp[MAXTOKEN_EVAL],itoken,tisvariable(char*),tisint(char*);  /* integers to read */
float ftemp[MAXTOKEN_EVAL]; /* floats to read */
...
int atom(float, float, float, int, float, float, float, float, char*),bond(int, int, float, float),angle(int, int, int, float, float),torsion(int, int,int,int,float,int,float),a_readvelocity(int, float, float, float);
int restrain(int, int,float, float),v_restrain(float*, float),f_restrain(float);
int tether(int,float,float,float,float),v_tether(float*, float),f_tether(float),alltether(float);
int v_ho_tether(float*, float),f_ho_tether(float);
int hybrid(int,int,int, int, float, float),v_hybrid(float*, float),f_hybrid(float);
int noel(int, int, float, float, float, float, float),v_noel(float*, float),f_noel(float);
int v_ho_noel(float*, float),f_ho_noel(float);
int math(char(*)[80],float[],int[],FILE*,FILE*,int);
int tset(FILE *, int, int, int, int, int, float);
int tmin( FILE *,int,int,int,int,int,int, int(*[])(float*, float),int);
#ifdef TIME
clock_t clock();
#endif
int significance();
void gsdg();
void analyze();
void AMMPmonitor(int(*[])(float*, float),int(*[])(float), int, FILE*);
void AMMPmonitor_mute(int(*[])(float*, float),int(*[])(float), int, FILE*);
void mom_add(int, int),mom(FILE*, float, int);
void tailor_qab();
void tailor_include();
void tailor_exclude();
int verlet(),v_maxwell(float, float, float, float),v_rescale(float);
int pac(),pacpac(), tpac(int (*[])(float), int,int,float,float),hpac(),ppac();
int ptpac();
...
/* for safety and to avoid side effects the token arrays are zero'd */
 	for( itoken=0; itoken<MAXTOKEN_EVAL; itoken++)
	{
		token[itoken][0] = '\0';
...
*/ 
	...
	{
	tmap( op,echo,potentials,nused, itemp[1],itemp[2],itemp[3],itemp[4],itemp[5],itemp[6],itemp[7],itemp[8],itemp[9],itemp[10]);
	goto DONE ;
	}
...
/* if a non-zero version then write it out */
	...
	{
	for( itoken=1; itoken<MAXTOKEN_EVAL; itoken++)
...
{
	char line[256], *fgets(char[], int, FILE*) ;
	char *sp,*wp;
