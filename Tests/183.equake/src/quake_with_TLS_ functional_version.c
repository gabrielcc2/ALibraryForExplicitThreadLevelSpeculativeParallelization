...
loop_speculator time_integration_loop_speculator, time_integration_loop_speculator_2, time_integration_loop_speculator_smvp;
...
/* Time integration loop */
  ...
  for (iter = 1; iter <= timesteps; iter++) {
	...
	time_integration_loop_speculator.speculate((void*&)shared_data, input_functions, input_vars);
	for (int aux_k = 1; aux_k < 4; aux_k++){
		time_integration_loop_speculator.append(initialize, (void*)aux_k);
	}
        time_integration_loop_speculator.commit(); 
	time_integration_loop_speculator.get_results();
	//input_functions.clear();
	//input_functions.push_back(smvp_for_spec);

	//int input_var=0;

	//for (input_var=0; input_var<ceil(ARCHnodes/10); input_var++){
	...
	/*	input_vars.clear();
		input_var_ten=input_var*10;
		input_vars.push_back((void*)input_var_ten);	//deberia ser input_var

		time_integration_loop_speculator_smvp.speculate((void*&)ptr_to_disp, input_functions, input_vars);

		cout<<input_var_ten<<" "<<floor((double)input_var_ten/(double)10)<<endl;

  		for (i = (input_var*10)+1; i < (input_var+1)*10; i++){//ARCHnodes; i++) {
			if (i<ARCHnodes){
				time_integration_loop_speculator_smvp.append(smvp_for_spec, (void*)i);
				cout<<i<<" "<<floor((double)i/(double)10)<<endl;
			}

		}
		time_integration_loop_speculator_smvp.commit();
		cout<<"Res: "<<input_var<<": "<<time_integration_loop_speculator_smvp.get_results()<<endl;
	}*/
	...
	time_integration_loop_speculator_2.speculate((void*&)shared_data, input_functions, input_vars);
	 
   	for (i = 1; i < 4; i++){
		time_integration_loop_speculator_2.append(second_part_of_time_integration_loop, (void*)i);
	}
        time_integration_loop_speculator_2.commit(); 
        time_integration_loop_speculator_2.get_results();
...
void smvp (int nodes, double ***A, int *Acol, int *Aindex, double **v, double **w)
{
  ...
  for (i = 0; i < nodes; i++) {
    ...
    Anext = Aindex[i];
    Alast = Aindex[i + 1];
    sum0 = A[Anext][0][0] * vi0 + A[Anext][0][1] * vi1 + A[Anext][0][2] * vi2;
    sum1 = A[Anext][1][0] * vi0 + A[Anext][1][1] * vi1 + A[Anext][1][2] * vi2;
    sum2 = A[Anext][2][0] * vi0 + A[Anext][2][1] * vi1 + A[Anext][2][2] * vi2;
    Anext++;
    while (Anext < Alast) {
      col = Acol[Anext];
      vcol0 = v[col][0];
      ...
      value = A[Anext][0][0];
      sum0 += value * vcol0;
      wcol0 = w[col][0] + value * vi0; //LE1 DR
      value = A[Anext][0][1];
      sum0 += value * vcol1;
      wcol1 = w[col][1] + value * vi0; //LE2 DR
      value = A[Anext][0][2];
      sum0 += value * vcol2;
      wcol2 = w[col][2] + value * vi0; //LE3 DR
      value = A[Anext][1][0];
      sum1 += value * vcol0;
      wcol0 += value * vi1;
      value = A[Anext][1][1];
      ...
      value = A[Anext][2][0];
      sum2 += value * vcol0;
      w[col][0] = wcol0 + value * vi2; //EE1DA
      value = A[Anext][2][1];
      sum2 += value * vcol1;
      w[col][1] = wcol1 + value * vi2; //EE2DA
      value = A[Anext][2][2];
      sum2 += value * vcol2;
      w[col][2] = wcol2 + value * vi2; //EE3 DA
      Anext++;
    }
    w[i][0] += sum0; //LE4 EE4... 
    w[i][1] += sum1;//LE4 EE4 
