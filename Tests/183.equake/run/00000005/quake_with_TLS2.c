...
/*loop _loop_in_batches declared globally*/
parallel_loop_in_batches time_integration_loop_in_batches, time_integration_loop_in_batches_2, time_integration_loop_in_batches_smvp; //[3017];
...
/*	for (int input_var=0; input_var<ARCHnodes+10000; input_var+=10000){
		if (input_var<ARCHnodes){
			...
			for (int aux_i=(input_var)+1000; aux_i<(input_var+10000); aux_i+=1000){
				if (aux_i<ARCHnodes){
					while (time_integration_loop_in_batches_smvp.append(smvp_for_spec, (void*)aux_i)!=0){ //[input_var/10]
...
/*      if (col>=top){
	cout<<"Dependence violation: it:"<<i<<" col: "<<col<<endl;
      }*/
...
//      pthread_mutex_lock(&col_mutex[col]);
      disp[disptplus][col][0] += wcol0 + value * vi2; //LE EE
      value = K[Anext][2][1];
      sum2 += value * vcol1;
      disp[disptplus][col][1] += wcol1 + value * vi2; //LE EE
      value = K[Anext][2][2];
      sum2 += value * vcol2;
      disp[disptplus][col][2] += wcol2 + value * vi2; //LE EE
  //    pthread_mutex_unlock(&col_mutex[col]);
      Anext++;
    }
    //pthread_mutex_lock(&col_mutex[i]);
    disp[disptplus][i][0] += sum0; //EE
    disp[disptplus][i][1] += sum1; //EE
    disp[disptplus][i][2] += sum2; //EE
...
/*    Anext = ARCHmatrixindex[i];
    ...
    sum0 = K[Anext][0][0]*disp[dispt][i][0] + K[Anext][0][1]*disp[dispt][i][1] + K[Anext][0][2]*disp[dispt][i][2];
    sum1 = K[Anext][1][0]*disp[dispt][i][0] + K[Anext][1][1]*disp[dispt][i][1] + K[Anext][1][2]*disp[dispt][i][2];
    sum2 = K[Anext][2][0]*disp[dispt][i][0] + K[Anext][2][1]*disp[dispt][i][1] + K[Anext][2][2]*disp[dispt][i][2];

    Anext++;
    while (Anext < Alast) {
      col = ARCHmatrixcol[Anext];

      sum0 += K[Anext][0][0]*disp[dispt][col][0] + K[Anext][0][1]*disp[dispt][col][1] + K[Anext][0][2]*disp[dispt][col][2];
      sum1 += K[Anext][1][0]*disp[dispt][col][0] + K[Anext][1][1]*disp[dispt][col][1] + K[Anext][1][2]*disp[dispt][col][2];
      sum2 += K[Anext][2][0]*disp[dispt][col][0] + K[Anext][2][1]*disp[dispt][col][1] + K[Anext][2][2]*disp[dispt][col][2];
      

      disp[disptplus][col][0] += K[Anext][0][0]*disp[dispt][i][0] + K[Anext][1][0]*disp[dispt][i][1] + K[Anext][2][0]*disp[dispt][i][2];
      disp[disptplus][col][1] += K[Anext][0][1]*disp[dispt][i][0] + K[Anext][1][1]*disp[dispt][i][1] + K[Anext][2][1]*disp[dispt][i][2];
      disp[disptplus][col][2] += K[Anext][0][2]*disp[dispt][i][0] + K[Anext][1][2]*disp[dispt][i][1] + K[Anext][2][2]*disp[dispt][i][2];
      Anext++;
    }
    disp[disptplus][i][0] += sum0;
    disp[disptplus][i][1] += sum1;
    disp[disptplus][i][2] += sum2;*/
