		    ...
		    GLenum type, struct gl_image *image )
{
   ...
   if (target==GL_TEXTURE_1D) {
      struct gl_texture_image *teximage;
      if (texture_1d_error_check( ctx, target, level, (GLenum) internalformat,
      ...
      if (image) {
         teximage = image_to_texture(ctx, image, (GLenum) internalformat, border);
      }
      else {
                    ...
                    struct gl_image *image )
{
   ...
   if (target==GL_TEXTURE_2D) {
      struct gl_texture_image *teximage;
      if (texture_2d_error_check( ctx, target, level, (GLenum) internalformat,
                       ...
                       struct gl_image *image )
{
   ...
   if (target==GL_TEXTURE_3D_EXT) {
      struct gl_texture_image *teximage;
      if (texture_3d_error_check( ctx, target, level, (GLenum) internalformat,
