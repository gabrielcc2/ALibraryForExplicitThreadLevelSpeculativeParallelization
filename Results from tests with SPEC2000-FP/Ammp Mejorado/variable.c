...
*/
VARIABLE *match_variable(char *name)
{
	VARIABLE *vp;
	vp = variableFIRST; 
	while(1 )
...
{
	VARIABLE *new_p;
	VARIABLE *match_variable(char*);
	int i;
	
	new_p = match_variable(name);
        if( new_p == NULL)
        {
        if( ( new_p = (VARIABLE*) malloc(  variableLONG ) ) == NULL)
        {
        return 0;
        }
        new_p->next = NULL;
        /* initialize the pointers */
        if( variableFIRST == NULL) variableFIRST = new_p;
        if( variableLAST == NULL) variableLAST = new_p;
	}
	new_p->value.f = f;
	new_p->type = FLOAT;
	for( i=0; i< NUM_TOT; i++)
