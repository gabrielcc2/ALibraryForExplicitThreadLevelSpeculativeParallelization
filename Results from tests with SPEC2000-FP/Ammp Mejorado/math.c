...
extern int (*potentials[10])(float*, float),(*forces[10])(float),nused;
...
int math(char tokens[][TOKENLENGTH], float fvalue [], int ivalue[], FILE* ip, FILE* op, int echo)
#ifdef ESV
...
VARIABLE *vp,*vos,*match_variable(char*);
float *getatomdata(char*); /* return the address of the atomdata if valid */
int validatom(char*); /* return 0 iff not a valid atomdata format */

/*SPEC don't declare set_f_variable here, do it in ammp.h jh/9/21/99 */
/* int set_f_variable(), set_i_variable(); */
int set_i_variable(char*, int); 

int a_number(),tisint(char*);
ATOM *ap,*a_next(int),*a_m_serial(int);
int i,j;
float a_l2_f(),a_max_f(),a_max_d();
float linmin(int (*[10])(float*, float), int, float); /* defined in optimist.c */
...
*/
float *getatomdata(char *who)
{
int validatom(char*);
int i,j;
char aser[TOKENLENGTH],*cp;
ATOM *ap,*a_m_serial(int);
VARIABLE *vp,*match_variable(char*);
