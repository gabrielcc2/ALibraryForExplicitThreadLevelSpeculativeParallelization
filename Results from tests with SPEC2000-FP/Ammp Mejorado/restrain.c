...
* returns
*/
int restrain(int p1, int p2, float bl, float fk)
	{
...
/* check to see if a restraint is already defined */
	...
	while(1)
	{
		...
		{
		new_p->length = bl; new_p->k = fk; return 1;
		}
		if( new_p == (RESTRAIN*)new_p->next) break;
		new_p = (RESTRAIN*) new_p->next;
	}
	}
	if( ( new_p = (RESTRAIN*) malloc( RLONG ) ) == NULL)
	{
	return 0;
	}
	/* initialize the pointers */
	if( restrain_first == NULL) restrain_first = new_p;
	if( restrain_last == NULL) restrain_last = new_p;
	new_p -> atom1 = ap1;
	new_p -> atom2 = ap2;
	new_p -> length = bl;
	new_p -> k = fk;
	new_p -> next = new_p;
	restrain_last -> next = new_p;
	restrain_last = new_p;
	return 1;
	}
...
* V is the potential */
int v_restrain(float *V, float lambda)
{
	RESTRAIN *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
	...
	if( bp == (RESTRAIN*) bp->next ) return 1;
	bp = (RESTRAIN*) bp->next;
       }
}
/* f_restrain()
...
void get_restrain(ATOM *a1, ATOM *restrained[], int mrestrain,int *inrestrain)
{
	...
	*inrestrain = 0;
	...
	if( mine == (RESTRAIN*) mine->next) return;
...
void dump_restrains( FILE *where )
{
	...
	b = restrain_first;
	if( b == NULL ) return;
	while( ((RESTRAIN*)b->next != b) )
	{
	if( (RESTRAIN*)b->next == NULL) return;
...
/* standard returns 0 if error (any) 1 if ok
* V is the potential */
int a_restrain(float *V, float lambda, int ilow, int ihigh, FILE *op)
{
	RESTRAIN *bp;
	float r,xt,yt,zt;
	ATOM *a1,*a2;
