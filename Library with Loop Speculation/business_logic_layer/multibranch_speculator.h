			...
			if (reset_all_logs && cancel){  //all the readers log will be deleted
				...
				if (!_spec_threads.empty()){
					if (valid_branch>=0 && valid_branch<_spec_threads.size()){				
						...
						pthread_mutex_destroy(&_spec_threads[valid_branch]._mutex);//This is a controversial decision.
					}
					_spec_threads.clear(); 
				}				
				...
				cancel have finished, and thus will not use the object*/
					...
					for (it=_thread_index.begin(); it!=_thread_index.end(); it++){
						if (it->second==-1){
							...
							//} while (a==0); //It could be done in a while, to guarantee termination. 
							//A proper decision requires a study of pthread-version standards regarding thread cancellation.
						}
					}
				...
				if (data_to_access.empty()){
					...
					if (!cancel) { //this means that the threads will be restarted
						...
						for (i=0; i<threads_to_delete.size(); i++){
							...
							//} while (success!=0); //It could be done in a while, to guarantee termination. 
							//A proper decision requires a study of pthread-version standards regarding thread cancellation.
...
public: 
	...
	int append(void* (f)(void*), void* consts){
		...
		//	} while (success!=0); //It could be done in a while. 
							//A proper decision requires a study of pthread-version standards regarding thread creation.
...
//		}while (success==0); //It could be done in a while. 
							//A proper decision requires a study of pthread-version standards regarding join delays.
...
//		} while (a==0); //It could be done in a while. 
				//A proper decision requires a study of pthread-version standards regarding join delays.
