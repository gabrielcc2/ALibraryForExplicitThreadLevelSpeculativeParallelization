...
void* initialize (void* arg){
	...
	for (int i=base; i<tope; i++){
		...
		while (b.write_data((void*&)aux, (void*)aux2, sizeof(bool))!=0){
		}
	}
	b.commit();
};
...
void* initialize_c (void* arg){
	...
	for (int i=base; i<tope; i++){
		...
		c.write_data((void*&)aux, (void*)aux2, sizeof(bool));
	}
	c.commit();
};
...
int main()
{
	...
	b.speculate((void*&)argumento, auxiliar, constA, SCHED_RR); //Se manda a especular con solo la primera iteración.
	...
	c.speculate((void*&)argumento, auxiliar, constA, SCHED_RR); //Se manda a especular con solo la primera iteración.
	for (int i=3; i<num_primes; i++){
		c.append(get_primes, (void*)i); //Se añaden las demás iteraciones y argumentos.
	}
	...
	constA.clear();
	constA.push_back((void*)2); //Argumento de primera iteración.
	cout<<"Inicio segunda especulaciòn..."<<endl;
	c.speculate((void*&)argumento, auxiliar, constA, SCHED_RR); //Se manda a especular con solo la primera iteración.
	for (int i=3; i<num_primes; i++){
